...
void FastExplorationFSM::pubState() {
  ...
  // 发布自动开始状态信息
  if (fp_->enable_auto_start_ && state_ == WAIT_TRIGGER && !fd_->auto_start_triggered_) {
    std_msgs::String auto_status_msg;
    ...
    oss << "condition: " << fp_->auto_start_condition_ 
        << ", elapsed: " << std::fixed << std::setprecision(1) << elapsed << "/" << fp_->auto_start_delay_ << "s"
        << ", odom: " << (odom_ready ? "ready" : "waiting")
        << ", topo: " << (topo_ready ? "ready" : "waiting");
    auto_status_msg.data = oss.str();
...
bool FastExplorationFSM::checkAutoStartCondition() {
  double elapsed_time = (ros::Time::now() - fd_->system_start_time_).toSec();
  
  if (fp_->auto_start_condition_ == "odom_ready") {
    // 基于里程计准备状态的自动开始
    bool odom_condition = fd_->have_odom_;
    bool delay_condition = elapsed_time >= fp_->auto_start_delay_;  // 使用配置的延迟时间确保系统稳定
    return odom_condition && delay_condition;
  } 
  else if (fp_->auto_start_condition_ == "map_ready") {
    // 基于地图初始化完成的自动开始
    bool odom_condition = fd_->have_odom_;
    bool topo_condition = !planner_manager_->topo_graph_->odom_node_->neighbors_.empty();
    bool delay_condition = elapsed_time >= fp_->auto_start_delay_;  // 使用配置的延迟时间确保地图初始化
    return odom_condition && topo_condition && delay_condition;
  }
  
  // 默认情况：使用odom_ready条件
  ROS_WARN_THROTTLE(5.0, "[Auto Start] Unknown condition '%s', using odom_ready condition", 
                    fp_->auto_start_condition_.c_str());
  bool odom_condition = fd_->have_odom_;
