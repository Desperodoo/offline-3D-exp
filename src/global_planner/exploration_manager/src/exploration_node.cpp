...
int main(int argc, char **argv) {
  ...
  // 创建探索统计模块
  ExplorationStatistics::Ptr exploration_stats = std::make_shared<ExplorationStatistics>();
  bool stats_enabled = false;
  nh.param("exploration_stats/enable", stats_enabled, false);
  
  if (stats_enabled) {
    exploration_stats->initialize(nh);
    explore_manager->setExplorationStats(exploration_stats);
    ROS_INFO("✓ Exploration statistics module initialized and linked");
  } else {
    ROS_INFO("⚠️ Exploration statistics module disabled");
  }

  // 集成拓扑图提取器 (已在explore_manager中创建)
  ROS_INFO("Topo graph extractor initialized via exploration manager");
