...
public:
  ...
  // 拓扑图提取器
  std::shared_ptr<TopoExtractorIntegrated> topo_extractor_;
  ...
  // 设置探索统计模块
  void setExplorationStats(ExplorationStatistics::Ptr stats) {
    exploration_stats_ = stats;
    if (topo_extractor_) {
      topo_extractor_->setExplorationStats(stats);
    }
  }
