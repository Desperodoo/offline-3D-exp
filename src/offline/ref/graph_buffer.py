...
def load_graph_dataset(file_path: str, buffer_size: int = None, batch_size: int = 256, 
                      device: str = "cuda", nstep: int = 1) -> EfficientGraphReplayBuffer:
    """
    从磁盘加载离线数据集到图缓冲区
    
    参数:
        file_path: 数据文件路径
        buffer_size: 可选，缓冲区大小，默认使用数据集大小
        batch_size: 批量大小
        device: 设备
        nstep: n步回报的n值
        
    返回:
        加载了数据的EfficientGraphReplayBuffer实例
    """
    with h5py.File(file_path, 'r') as f:
        # 获取数据文件中的字段信息
        all_keys = list(f.keys())
        # print(f"数据文件包含以下字段: {all_keys}")
        ...
        adj_list = f['adj_list'][:]
        actions = f['actions'][:]  # 目前的数据的字段名是action，后面会统一成actions，rewards和dones
        # logp = f['logp'][:] if 'logp' in f else np.zeros(len(actions))
        rewards = f['rewards'][:]
        dones = f['dones'][:]
        
        # 获取数据集大小和维度信息
        data_size = len(rewards)
        node_dim = node_inputs.shape[-1]
        node_padding_size = node_inputs.shape[1]
        viewpoint_padding_size = viewpoints.shape[1]
        k_size = adj_list.shape[-1]
        
        print(f"数据集信息: 样本数={data_size}, 节点维度={node_dim}, 节点数={node_padding_size}")
        
        # 如果未指定buffer_size，使用数据集大小
        if buffer_size is None or buffer_size > data_size:
            buffer_size = data_size
        
        # 创建缓冲区
        buffer = EfficientGraphReplayBuffer(
        ...
        # 加载数据到缓冲区
        for i in range(min(buffer_size, data_size)):
            # 创建时间步
            time_step = GraphTimeStep(
                node_inputs=node_inputs[i],
                node_padding_mask=node_padding_mask[i],
                current_index=current_index[i],
                viewpoints=viewpoints[i],
                viewpoint_padding_mask=viewpoint_padding_mask[i],
                adj_list=adj_list[i],
                action=actions[i],
                # logp=logp[i],
                reward=rewards[i],
                done=dones[i],
                first=(i > 0 and dones[i-1]) or (i == 0)  # 估计first标志
            )
            buffer.add(time_step)
            
        return buffer

def load_batch_data_to_buffer(batch_file, buffer_size=None, batch_size=256, device="cuda"):
    """
    从单个批次文件加载数据到图结构缓冲区
    """
    file_path = batch_file['path'] if isinstance(batch_file, dict) else batch_file
    
    print(f"从文件加载数据到缓冲区: {file_path}")
    
    # 使用原始的load_graph_dataset函数，不传入rank和world_size
    buffer = load_graph_dataset(
        file_path,
        buffer_size=buffer_size,
        batch_size=batch_size,
        device=device
    ...
    print(f"成功加载到缓冲区，样本数: {len(buffer)}")
    return buffer
